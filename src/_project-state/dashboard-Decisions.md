...
## 2026-06-28 — B.08 accessibility & polish choices

Choices made during B.08 that the phase prompt did not spell out. None reverses a locked decision; all serve the phase's Definition of Done (keyboard operability, semantics, announced states, mobile, WCAG 2.1 AA contrast, reduced motion, offline axe tests). No runtime dependency, env var, route, or security/write-path file changed.

- **Offline a11y test stack: `jest-axe` (not `vitest-axe`), `@testing-library/react`/`-dom`/`-user-event`, `jsdom` — all dev-only, pinned exact.** `jest-axe@10.0.0` is the most stable, runner-agnostic axe matcher (it bundles `axe-core` and registers via `expect.extend`, so it works under Vitest 4 with a one-file type augmentation: `test/setup/vitest-axe.d.ts`); `vitest-axe@0.1.0` is far less established. RTL 16.3.2 is the React-19-compatible line. `@testing-library/user-event@14.6.1` drives the keyboard-interaction tests (tab roving, dialog Escape). Versions: `jsdom@29.1.1`, `@testing-library/react@16.3.2`, `@testing-library/dom@10.4.1`, `@testing-library/user-event@14.6.1`, `jest-axe@10.0.0`, `@types/jest-axe@3.5.9`.
- **Two Vitest projects share one alias set (replacing the single Node config).** `vitest.config.ts` now defines a **unit** project (`*.test.ts`, Node — the unchanged B.03–B.07 logic/crypto/isolation suites) and an **a11y** project (`*.test.tsx`, jsdom, with `test/setup/a11y-setup.ts`). This keeps the Node tests in Node (the crypto module needs `node:crypto`) while the component tests get a DOM, without per-file environment hacks. Chosen over `environmentMatchGlobs` (removed in Vitest 4).
- **axe assertions are scoped to the WCAG 2.1 A/AA tags** (`wcag2a, wcag2aa, wcag21a, wcag21aa`). The default axe ruleset also runs page-level *best-practice* rules ("page must have one h1", "all content in landmarks") that are correct for a whole document but spurious for an isolated component fragment. The DoD's bar is WCAG 2.1 AA, so the runs target exactly that. **Colour-contrast is verified separately** (see next bullet): jsdom doesn't apply the stylesheet, so axe reports contrast as "incomplete," never as a pass or a violation — it cannot be the contrast oracle.
- **Contrast fixes are lightness-only adjustments within the existing grayscale / same hue — not a new palette.** Verified every text/interactive pair against the dark tokens with a standalone WCAG contrast calculator. Two shadcn-bridge tokens failed AA for normal text and were lightened: `--muted-foreground` 45% → **58%** (was 3.2–3.9:1 on the background/card surfaces; now 5.0–6.1:1) and `--destructive` 60% → **66%** (error text was 4.0–4.4:1 on the login card / delete popover; now ≥4.7:1 everywhere it renders). The two supplemental status hints lost their `/70` alpha (`text-muted-foreground/70` → `text-muted-foreground`) so "· Edited" / "· Unpublished edits" also clear 4.5:1. This honors "adjust only token application where something fails, not the palette": no new hue is introduced, the grays stay gray, and the brand reads unchanged — it is the minimum change to meet the AA requirement the DoD mandates.
- **The delete-confirm became a real modal dialog (`@base-ui/react` Alert Dialog), replacing the inline reveal.** The DoD requires the delete-confirm to trap focus, close on Escape, and return focus to its trigger — behaviors an inline reveal does not have. The Alert Dialog primitive provides all three for free (plus `role="alertdialog"` + `aria-modal`, programmatic title/description association, and pointer-non-dismissable semantics for a destructive action). As a bonus it satisfies "disable conflicting controls while busy": while the confirmation is open the rest of the editor is inert, so Save/Publish can't be operated mid-delete. A new shadcn-style wrapper lives at `src/components/ui/alert-dialog.tsx` (a `ui/` primitive, explicitly permitted by the phase scope). `@base-ui/react` was already a dependency — **no new dependency**.
- **Locale tabs are a hand-rolled WAI-ARIA tablist, not the base-ui Tabs primitive.** The editor must keep **every** locale's field group mounted so a multi-locale submit carries all languages; the Tabs primitive unmounts inactive panels, which would drop the hidden locales' inputs from the form. So the tablist implements the same contract manually: roving `tabindex` (only the selected tab is tabbable), Left/Right arrow navigation with wrap + Home/End, and `aria-controls`/`aria-labelledby` association — while the panels stay mounted and only toggle `hidden`.
- **Added a route-level loading state (`LoadingState` + three `loading.tsx`).** The DoD lists "loading" among the states that must render consistently, but the portal had none — the `force-dynamic` pages fetch from Sanity with no boundary. `src/components/portal/loading-state.tsx` is a polite `role="status"` skeleton (branded spinner + visible text), wired via `loading.tsx` for `/posts`, `/posts/new`, `/posts/[id]`. Leaks nothing (label-only).
- **Extracted a shared `MessageCard` for the friendly states.** The posts page used an inline `StateCard` while the editor pages used `EditorMessage`; both now render through one `src/components/portal/message-card.tsx`, so empty / not-linked / not-ready / read-error / not-found look consistent (Task 4). It takes a `headingLevel` (1 when the card *is* the page — the editor message screens; 2 when it sits under a page `<h1>` — the posts list) so every page has exactly one `<h1>` with a sensible order.
- **Other polish:** a skip-to-main-content link on the portal shell (`.skip-link` + `<main id="main-content" tabIndex={-1}>`); a `prefers-reduced-motion` block that neutralizes the decorative entrances/transitions (busy is still conveyed by label + `aria-busy` + `aria-live`); visible pending **verbs** on the action buttons ("Saving…/Publishing…/Deleting…") with an `sr-only` `role="status"` region; `lang` on the non-UI-language (mk/es) content fields; ≥44px mobile tap targets on the primary actions (Save/Publish/Delete/New post via `h-11 sm:h-9`); focus-visible rings on the previously-bare "Back to posts" links; `aria-describedby` linking the login error to its inputs; and removal of the misleading non-interactive chevron on the top-bar account display (it implied a menu that doesn't exist).

**Decided by:** Claude, during B.08 implementation. Reverse any by adding a new dated entry.
