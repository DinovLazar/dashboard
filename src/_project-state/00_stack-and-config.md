...
The real, verified stack for the Vertex Consulting client blog portal as of **Phase B.08** (2026-06-28). Append changes as they land; keep it factual.

**Build status: building clean with auth + the registry data layer + the secure per-tenant Sanity read AND write path + featured-image upload (the config-driven editor), now accessibility-polished with offline axe tests.** `npm run build` passes (the `server-only` guard holds), `npm run lint` is clean, and `npm test` is green (**156 tests**: the 130 prior unit/isolation tests, unchanged, + **26 B.08 offline a11y/keyboard tests** — post editor (create/edit/multi-locale/with-image/no-image) + posts list (populated/empty) + `LoadingState` + `MessageCard`/`EditorMessage` states + the login form + the portal top bar, all with zero axe violations, plus keyboard proofs for the locale-tab roving, the delete-dialog Escape/focus-return, and the Save/Delete/image-upload pending-state announcements). The Supabase registry schema (three tables + RLS) lives in `supabase/migrations/`; the encrypted-token crypto module, the service-role client, the tenant resolver, the per-tenant Sanity read path, the **mutation module** (`src/lib/sanity/mutations.ts`), the **image-upload module** (`src/lib/sanity/assets.ts`), and the **Server Actions** (`src/app/(portal)/posts/actions.ts`) are in place. `/login`, `/posts`, `/posts/new`, and `/posts/[id]` are dynamic (`ƒ`), and the proxy is active. **B.08 added no new dependencies (only dev-only test deps), no env var, and no `next.config.ts` change**, and touched no `src/lib/**` security/write-path file. (B.06 made the first `next.config.ts` change since B.01 — image `remotePatterns` + `serverActions.bodySizeLimit`; B.07 was docs-only.)
...
**B.08 added (dev-only, offline a11y testing; pinned exact):** **`jsdom 29.1.1`** (DOM for the component-render tests), **`@testing-library/react 16.3.2`** + **`@testing-library/dom 10.4.1`** (render + queries; the React-19-compatible line), **`@testing-library/user-event 14.6.1`** (keyboard-interaction tests), **`jest-axe 10.0.0`** (axe matcher; bundles `axe-core`, registered on Vitest's `expect` via `expect.extend`) + **`@types/jest-axe 3.5.9`**. No runtime dependency, no new environment variable, no `next.config.ts` change.
...
- `vitest.config.ts` — **B.03** Vitest config: Node environment, `src/**/*.test.ts`, aliases `server-only`/`client-only` → the no-op stub; **B.04** also aliases `@` → `./src` (mirrors the `@/*` tsconfig path) so the cross-importing read-path modules resolve in tests; **B.08** restructured into two projects that share those aliases — **unit** (`*.test.ts`, Node) and **a11y** (`*.test.tsx`, jsdom + `setupFiles: ['./test/setup/a11y-setup.ts']`). Companion: `test/setup/a11y-setup.ts` (jsdom-only: axe matcher + RTL cleanup + browser-API stubs) and `test/setup/vitest-axe.d.ts` (matcher types)
